...
// 兼容历史数据：生成旧格式组合串（如：A,B,C）
const buildLegacyOptionCombination = (questions = [], answer = {}) => {
  return questions.map(question => answer[question.id]).join(',');
};

module.exports = {
  normalizeOptionCombination,
  buildStandardOptionCombination,
  buildLegacyOptionCombination
};
